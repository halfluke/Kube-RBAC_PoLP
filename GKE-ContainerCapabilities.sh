...
echo "[INFO] Known managed-plane pods (e.g. netd, kube-proxy, gke-metadata-server) keep their severity but are tagged [known GKE platform component] — not suppressed."
[[ "$ONLY_USER_NS" -eq 1 ]] && echo "Filtering: only non-system namespaces (not openshift-* / kube-system|kube-public|kube-node-lease / gke-* / gmp-* / config-management-system / gatekeeper-system)."
echo ""
...
# Annotate known managed-plane workloads (CNI/proxy/metadata). Never suppresses severity.
platform_workload_note () {
    local ns="$1" pod="$2" sa="$3"
    local note=""
    case "$sa" in
        kube-proxy|netd|gke-metadata-server|fluentbit-gke|gke-metrics-agent|pdcsi-node-sa|gke-gcsfuse-csi-driver)
            note=" [known GKE platform component — expected on managed nodes]"
            ;;
    esac
    if [[ -z "$note" ]]; then
        case "$pod" in
            kube-proxy-*|netd-*|gke-metadata-server-*|fluentbit-gke-*|gke-metrics-agent-*|pdcsi-node-*|gke-gcsfuse-driver-*)
                note=" [known GKE platform component — expected on managed nodes]"
                ;;
        esac
    fi
    if [[ -z "$note" ]] && { [[ "$ns" == "kube-system" || "$ns" == "kube-public" || "$ns" == "kube-node-lease" || "$ns" == gke-* || "$ns" == gmp-* ]] || [[ "${IS_SYSTEM_NS:-0}" -eq 1 ]]; }; then
...
while read -r NS; do
    ...
    IS_SYSTEM_NS=0
    # Reserved kube names only — not kube-* prefix (lab fixtures use kube-rbac-polp-*).
    if [[ "$NS" == openshift-* ]] \
    ...
    while read -r POD; do
        ...
        if [[ "$OUTPUT_MODE" == "text" ]]; then
            ...
            echo "\"${NS//\"/\"\"}\",\"${POD//\"/\"\"}\",\"${SA_NAME//\"/\"\"}\",\"$PRIV_DISPLAY\",\"${STATUS//\"/\"\"}\",\"${REQUESTED_CAPS_STR//\"/\"\"}\",\"${DROPPED_CAPS_STR//\"/\"\"}\",\"${EFFECTIVE_ESC}\",\"$ESC_DISPLAY\",\"$HOST_PID\",\"$HOST_NET\",\"$HOST_IPC\",\"${RUN_AS_NONROOT_DISPLAY//\"/\"\"}\",\"${RUN_AS_USER//\"/\"\"}\",\"${RUN_AS_GROUP_DISPLAY//\"/\"\"}\",\"$FSGROUP\",\"${SUPP_GROUPS//\"/\"\"}\",\"${VOLUME_TYPES_DISPLAY//\"/\"\"}\",\"${AUTOMOUNT_DISPLAY//\"/\"\"}\",\"${ESCAPE_ESC}\",\"${WI_GSA//\"/\"\"}\""
